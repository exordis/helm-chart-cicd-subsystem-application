...
docker run -ti --rm -v .:/src --entrypoint /bin/sh helmunittest/helm-unittest:3.16.3-0.7.0 -c "cp -R /src/* /apps && cd /apps && helm unittest . $args && rm -rf /src/tests/__snapshot__ && cp -R /apps/tests/__snapshot__ /src/tests/" 